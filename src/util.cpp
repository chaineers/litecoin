...
const char * const BITCOIN_CONF_FILENAME = "techroadcoin.conf";
const char * const BITCOIN_PID_FILENAME = "techroadcoind.pid";
...
#else
    const char* pszModule = "techroadcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Techroadcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Techroadcoin";
#else
    // Unix
    return pathRet / ".techroadcoin";
#endif
#endif
}
